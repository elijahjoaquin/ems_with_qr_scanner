...
<?php
	...
	{ 
		...
		$REMOVERTIME = $av_overtime - $num_hrs;
		...
		mysqli_query($conn,"INSERT INTO auditlog(Action, PostingDate, Actor_ID, Department, Role, Recipient_ID) VALUES('Admin Rejected Overtime', '$date', '$session_id', '$session_depart', 'Admin', '$did')         
		") or die(mysqli_error());
		
			$sql="update tblovertime set registra_remarks=:description,admin_status=:status,AdminRemarkDate=:admremarkdate where id=:did";
		...
		elseif ($status === '1') {
		        ...
		        $date = date('Y-m-d H:i:s');
		mysqli_query($conn,"INSERT INTO auditlog(Action, PostingDate, Actor_ID, Department, Role, Recipient_ID) VALUES('Admin Approved Overtime', '$date', '$session_id', '$session_depart', 'Admin', '$did')         
		") or die(mysqli_error());
		
				$result = mysqli_query($conn,"update tblovertime, tblemployees set tblovertime.registra_remarks='$description',tblovertime.admin_status='$status',tblovertime.AdminRemarkDate='$admremarkdate', tblemployees.Av_overtime='$REMOVERTIME' where tblovertime.empid = tblemployees.emp_id AND tblovertime.id='$did'");
