...
if(isset($_POST["export4"])){
 if(empty($_POST["fromDate"])){
  ...
  if(count($filterOrders)) {
	ob_end_clean();
	  $fileName = "overtime_export_employee_".date('Y-m-d') . ".csv";
	  header("Content-Description: File Transfer");
	  header("Content-Disposition: attachment; filename=$fileName");
	  header("Content-Type: application/csv;");
	  $file = fopen('php://output', 'w');
	  $header = array("Emp ID", "First Name", "Last Name", "Posting Date", "No. of Hours", "Date of Overtime", "Manager Status", "Admin Status");
	  fputcsv($file, $header);  
