...
if(isset($_POST["export3"])){
 if(empty($_POST["fromDate"])){
  ...
  if(count($filterOrders)) {
	ob_end_clean();
	  $fileName = "leave_export_manager_".date('Y-m-d') . ".csv";
	  header("Content-Description: File Transfer");
	  header("Content-Disposition: attachment; filename=$fileName");
	  header("Content-Type: application/csv;");
	  $file = fopen('php://output', 'w');
	  $header = array("Emp ID", "First Name", "Last Name", "Leave Type", "Posting Date", "From Date", "To Date", "No. of Days", "Admin Status");
	  fputcsv($file, $header);  
